...
/* Private  define ------------------------------------------------------------*/
/* Private  macro -------------------------------------------------------------*/
#define ODRIVE_CAN hcan2
/* Private  variables ---------------------------------------------------------*/
CAN_TX_Typedef TX;
CAN_RX_Typedef RX;
extern		axis_t odrive_friction[2];  // 发射摩擦轮
...
void odrv_Set_Axis_Requested_State(axis_t Axis, Axis_State state) {
    Set_TX_Param(Axis.AXIS_ID, SET_AXIS_REQUESTED_STATE, CAN_ID_STD, CAN_RTR_DATA, 4);
    ...
    CAN_Send_Packet(&ODRIVE_CAN, &TX);
}

void odrv_Set_Input_Vel(axis_t Axis, float vel, float torqueff) {
    Set_TX_Param(Axis.AXIS_ID, SET_INPUT_VEL, CAN_ID_STD, CAN_RTR_DATA, 8);
    uint8_t *ptrVel;
    ptrVel = (uint8_t *)&vel;
    uint8_t *ptrTor;
...
void odrv_Clear_Errors(axis_t Axis) {
    Set_TX_Param(Axis.AXIS_ID, CLEAR_ERRORS, CAN_ID_STD, CAN_RTR_DATA, 0);
    CAN_Send_Packet(&ODRIVE_CAN, &TX);
}

void odrv_Reboot_ODrive(axis_t Axis) {
    Set_TX_Param(Axis.AXIS_ID, REBOOT_ODRIVE, CAN_ID_STD, CAN_RTR_DATA, 0);
    CAN_Send_Packet(&ODRIVE_CAN, &TX);
}

void odrv_Set_Controller_Modes(axis_t Axis, Control_Mode ControlMode, Input_Mode InputMode) {
    Set_TX_Param(Axis.AXIS_ID, SET_CONTROLLER_MODES, CAN_ID_STD, CAN_RTR_DATA, 8);
...
void odrv_Set_Input_Pos(axis_t Axis, float Input_Pos, int Vel_FF, int Torque_FF) {
    Set_TX_Param(Axis.AXIS_ID, SET_INPUT_POS, CAN_ID_STD, CAN_RTR_DATA, 8);
    uint8_t *ptrPos;
...
void odrv_Get_Encoder_Count(axis_t Axis) {
    Set_TX_Param(Axis.AXIS_ID, GET_ENCODER_COUNT, CAN_ID_STD, CAN_RTR_REMOTE, 0);
    CAN_Send_Packet(&ODRIVE_CAN, &TX);
}

void odrv_Set_Input_Torque(axis_t Axis, float torque) {
    Set_TX_Param(Axis.AXIS_ID, SET_INPUT_TORQUE, CAN_ID_STD, CAN_RTR_DATA, 4);
    uint8_t *ptrTor;
...
void odrv_Get_Bus_Voltage_Current(axis_t Axis) {
    Set_TX_Param(Axis.AXIS_ID, GET_BUS_VOLTAGE_CURRENT, CAN_ID_STD, CAN_RTR_REMOTE, 0);
    CAN_Send_Packet(&ODRIVE_CAN, &TX);
}

void odrv_Get_IQ(axis_t Axis) {
    Set_TX_Param(Axis.AXIS_ID, GET_IQ, CAN_ID_STD, CAN_RTR_REMOTE, 0);
    CAN_Send_Packet(&ODRIVE_CAN, &TX);
}

void odrv_Set_Position_Gain(axis_t Axis, float pos_gain) {
    Set_TX_Param(Axis.AXIS_ID, SET_POSITION_GAIN, CAN_ID_STD, CAN_RTR_DATA, 4);
    uint8_t *ptrPos;
...
void odrv_Set_Vel_Gains(axis_t Axis, float Vel_Gain, float Vel_Int_Gain) {
    Set_TX_Param(Axis.AXIS_ID, SET_VEL_GAINS, CAN_ID_STD, CAN_RTR_DATA, 8);
    uint8_t *ptrVelGain;
...
void odrv_Set_Axis_Node_ID(axis_t Axis, uint32_t node_id) {
    Set_TX_Param(Axis.AXIS_ID, SET_AXIS_NODE_ID, CAN_ID_STD, CAN_RTR_DATA, 4);
    uint8_t *ptrNodeId;
...
void odrv_Set_Limits(axis_t Axis, float vel_lim, float curr_lim) {
    Set_TX_Param(Axis.AXIS_ID, SET_LIMITS, CAN_ID_STD, CAN_RTR_DATA, 8);
    uint8_t *ptrVelLim;
